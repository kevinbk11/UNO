        ...
        data=>{        
            ...
            if(this.clients.includes(data.id)){
                ...
                room.players.forEach(player=>{
                    this.nameToClient[player].emit('PlayerJoinEvent',builder.addData('name',data.name).addData('number',room.players.length+1).build())
                })
                room.players.push(data.name)
                this.socket.emit('JoinRoomRespondEvent',builder.addData('players',room.players).addData('roomID',room.roomID).build())
            }
